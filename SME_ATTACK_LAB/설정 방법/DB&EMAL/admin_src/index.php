...
if (isset($_GET['action'])) {
    ...
    if ($action === 'login') {
        ...
        if ($admin && (int)$admin['is_active'] === 1 && password_verify($pw, $admin['password_hash'])) {
            ...
            $login_successMsg = "[{$login_time}] 로그인 성공. | 로그인 계정 :
            {$admin['username']} | IP 주소 : {$ip_addr} |" . PHP_EOL;
            $log_dir = __DIR__ . '/logs';
            $log_path = $log_dir . '/login.log';
            if (!is_dir($log_dir)){
            mkdir ($log_dir, 0744, true);
            }
            // 로그 파일 위치 /var/www/html/logs/login.log
            file_put_contents($log_path, $login_successMsg, FILE_APPEND | LOCK_EX);

...
// === DB 설정 ===
$DB_HOST = getenv('DB_HOST') ?: 'host.docker.internal'; // 컨테이너면 host.docker.internal, WSL 내부 PHP면 127.0.0.1
...
if (isset($_GET['action'])) {
    ...
    if ($action === 'login') {
        ...
        if ($user === '' || $pw === '') {
            echo json_encode(['ok' => false, 'error' => '아이디와 비밀번호를 입력하세요.']);
            exit;
        }
        ...
        if ($admin && (int)$admin['is_active'] === 1 && password_verify($pw, $admin['password_hash'])) {
            ...
            $_SESSION['admin'] = $admin['username'];     // 기존 화면 호환용
            ...
            $pdo->prepare("
                INSERT INTO audit_logs (admin_id, action, target_type, ip_address)
